...
פרויקט אחד. הכתובת והמפתח האנונימי **מוטמעים בקוד ומשוכפלים** במקום להיות
מיובאים ממקום אחד: הכתובת ב-**17 קבצים**, והמפתח עצמו ב-**16** — `supabase/schema.sql` מכיל רק
את הכתובת, בתוך הערה.
...
⚠️ **הקובץ שבמאגר מיושן.** `notify.js` שולח **GET** עם הכל ב-query string, אבל
`doGet()` שב-`.gs` מטפל רק ב-`?test=1`; לוגיקת המטען יושבת ב-`doPost()`
שהאפליקציה **לעולם לא קוראת לו**. הפריסה החיה היא גרסה חדשה יותר שלא נשמרה
במאגר.

⚠️ `notify.js` מקודד גם את כתובת הממסר וגם את **כתובת ה-Gmail של הבעלים**
כברירות מחדל, ומריץ shim שדורס בכוח כל כתובת `script.google.com` שמורה.
השליחה היא `mode: 'no-cors'` — **כשל במסירה אינו נראה**.
...
| **`api.ipify.org`** | `src/lib/notify.js` → `getIp()` | ה-IP של החותם לא נרשם. נקרא **בכל חתימה** מדפדפן החותם, והוא הבסיס הראייתי לטענת ה-IP ב-`legal.html` |
